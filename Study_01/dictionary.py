...
# 1. 초기 주문서 생성
# 딕셔너리를 사용하여 주문을 저장합니다.
# 키(Key): 손님 이름, 값(Value): 주문한 라면 종류
order = {'스페이드1': '비빔라면', '다이아': '매운라면'}

# 현재 주문서 출력
print(order)
# 출력: {'스페이드1': '비빔라면', '다이아': '매운라면'}
...
# 주문이 추가된 후 출력
print(order)
# 출력: {'스페이드1': '비빔라면', '다이아': '매운라면', '클로버3': '카레라면'}

# 3. 기존 주문 변경
# '다이아'가 기존 '매운라면' 주문을 '짜장라면'으로 변경
order['다이아'] = '짜장라면'

# 변경된 주문서 출력
print(order)
# 출력: {'스페이드1': '비빔라면', '다이아': '짜장라면', '클로버3': '카레라면'}

# 4. 주문 취소
# '스페이드1'이 주문을 취소 (딕셔너리에서 삭제)
del order['스페이드1']

# 최종 주문서 출력
print(order)
# 출력: {'다이아': '짜장라면', '클로버3': '카레라면'}

# 👉 결론: 딕셔너리를 활용하면 데이터를 동적으로 추가, 수정, 삭제할 수 있어 주문 관리에 유용함.
